...
うましっぽと**同じ管理システム**（https://affiliate.tairiku-tsusho.co.jp ）に成果を集約し、
...
> - **サイトコード**：サイトを区別する短い識別子。うましっぽ=`umashippo`／馬肉特急=`baniku`。

---

## 0. 前提の確認

- 馬肉特急のEC-CUBEが **4.x系**であること（うましっぽと同じ4.3系なら、プラグインはそのまま動きます）
...
プラグインの中身は、うましっぽとまったく同一です。入手方法は以下のいずれか。

### 方法A：うましっぽサーバーからコピー（推奨・確実）
うましっぽサーバーの以下のフォルダを丸ごとコピーして、馬肉特急のEC-CUBEの同じ場所に置きます。

```
コピー元（うましっぽ）: ~/shizenha-inu.life/public_html/app/Plugin/Affiliate/
...
**`AFFILIATE_SITE_CODE=baniku` が最重要**（これが無いとうましっぽとして記録されてしまいます）。

```dotenv
AFFILIATE_API_URL=https://affiliate.tairiku-tsusho.co.jp
AFFILIATE_API_KEY=（うましっぽの .env と同じ値をコピー）
AFFILIATE_COOKIE_DAYS=30
AFFILIATE_TRACK_CLICKS=true
AFFILIATE_SITE_CODE=baniku
```

- **AFFILIATE_API_KEY**：うましっぽサーバーの `~/shizenha-inu.life/public_html/.env` にある
...
> （うましっぽサーバーには `.env.local.php` は無く、`.env` を直接読む構成でした。同じ構成なら不要です）
...
> うましっぽサーバーではこの方法で有効化しています。もし `eccube:plugin:install` でエラーが出る場合は、
> うましっぽの導入時と同じ対応が必要になることがあります（不明点はご連絡ください）。
