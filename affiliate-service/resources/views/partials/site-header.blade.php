{{--
  ...
  馬肉特急・自然派いぬ生活ともに文字、先頭に「馬」マーク、末尾に「アンバサダー」。
...
<style>
  ...
  .site-header__inner{max-width:1180px;margin:0 auto;padding:11px clamp(18px,5vw,56px);display:flex;align-items:center;justify-content:space-between;gap:16px;}
  ...
  .site-header__name{font-family:'Shippori Mincho',serif;font-weight:600;font-size:clamp(13px,1.5vw,15px);letter-spacing:0.04em;color:#f4eee1;white-space:nowrap;}
  ...
  @media (max-width:575.98px){ .site-header__name{font-size:12.5px;} .site-header__badge{width:30px;height:30px;font-size:14px;} }
</style>
<header class="site-header">
  <div class="site-header__inner">
    <a href="{{ route('home') }}" class="site-header__brand">
      <span class="site-header__badge" aria-hidden="true">馬</span>
      <span class="site-header__name">馬肉特急 <span class="sep">·</span> 自然派いぬ生活 <span class="amb">アンバサダー</span></span>
    </a>
    <nav class="site-header__nav">
      @if (session('affiliate_authenticated'))
        <a class="sh-link" href="{{ route('affiliate.mypage') }}">マイページ</a>
        <a class="sh-link" href="{{ route('inquiry.create') }}">お問い合わせ</a>
